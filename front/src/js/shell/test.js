...
export const runTests = () => {
  ...
  it("correctly builds a tree with children", () => {
    ...
    assert(
      Object.keys(root.children.a.children).length === 1,
      "a has one child"
    );
    assert(
      Object.keys(root.children.a.children.b.children).length === 4,
      "b has four children"
    );
    assert(
      Object.keys(root.children.a.children.b.children.e.children).length === 2,
      "e has two children"
    );
  });
  ...
  it("builds with reference to parent", () => {
    ...
    assert(
      root.children.a.children.b.parent === root.children.a,
      "/ -> a -> b <- a"
    );
    assert(
      root.children.a.children.b.children.c.parent ===
        root.children.a.children.b,
      "/ -> a -> b -> c <- b"
    );
    assert(
      root.children.a.children.b.children.c.parent.parent.parent === root,
      "/ -> a -> b -> c <- b <- a <- /"
    );
  });
  ...
  it("inserts new nodes correctly", () => {
    ...
    assert(
      home_bruh_failure.error,
      "Trying to insert a node at a conflicting path fails"
    );
  });
  ...
  it("returns correct nodes with paths", () => {
    ...
    assert(
      fs.getNode("/a/b/../../.") === root,
      "Using . and .. returns correct path"
    );
    ...
    assert(
      fs.getNode("/a/../a/./").getFullPath() === "/a",
      "getFullPath reduces path"
    );
    assert(
      fs.getNode("/a/../a/./../..").getFullPath() === "/",
      "getFullPath reduces path on recursive parent root node"
    );
  });
  ...
  it("returns the correct status of a dir/file", () => {
    const fs = new FileSystem(testFs);
    assert(
      fs.pathStatus("/a/b/c", "file").node.name === "c",
      "File status is correct"
    );
    assert(
      fs.pathStatus("/a/b", "file").error,
      "Path is a dir, fails on file status"
    );
    assert(
      fs.pathStatus("/a/b", "file").node,
      "Path is a dir, fails on file status, still returns node"
    );
    assert(
      fs.pathStatus("/g/h").type === "file",
      "When no type specified, returns node and file type"
    );
    assert(
      fs.pathStatus("/asdfasdfasdfasdf").error,
      "Path does not exist, fails on status"
    );
  });
  ...
  it("has fileContents when it is a file", () => {
    const fs = new FileSystem(testFs);
    assert(
      fs.getNode("/a/b/c").fileContents === "This is a file",
      "File contents are correct"
    );
    assert(
      fs.getNode("/a/b/d").fileContents === "This is another file",
      "File contents are correct"
    );
    assert(
      fs.getNode("/a/b/e/f").fileContents === "This is a file",
      "File contents are correct"
    );
    assert(
      typeof fs.getNode("/g/h").fileContents === "function",
      "File contents are correct"
    );
  });
  ...
  it("deletes a directory or file", () => {
    ...
    assert(
      Object.keys(root.children.g.children).length === 1,
      "g has one child"
    );
    fs.getNode("/g/h").delete();
    assert(
      Object.keys(root.children.g.children).length === 0,
      "g has no children"
    );
  });
  ...
  it("parses commands correctly", () => {
    ...
    assert(
      arrayEqual(Shell.parseCommand("ls /"), ["ls", "/"]),
      "Parses ls command correctly"
    );
    assert(
      arrayEqual(
  ...
  it("executes commands in path and PWD", () => {
    ...
    assert(
      shell.run("h").streams.stdout === "Hello, world",
      "'h' program executed from PATH"
    );
    assert(
      shell.run("echo Hello, world").streams.stdout === "Hello, world",
      "'echo Hello, world' program executed from PATH"
    );
    shell.setEnv("PWD", "/a/b/e");
    assert(
      shell.run("./yes 5").streams.stdout === "yes yes yes yes yes",
      "'./yes 5' executed from PWD"
    );
    shell.setEnv("PWD", "/");
    assert(
      shell.run("/a/b/e/yes 5").streams.stdout === "yes yes yes yes yes",
      "'/a/b/e/yes 5' execute from absolute path"
    );
  });
  ...
  it("replaces environment variables in command", () => {
    ...
    assert(
      shell.run("echo $PWD").streams.stdout === "/",
      "PWD env var is correct"
    );
  });
  ...
  it("sets environment variables from command line", () => {
    ...
    assert(
      shell.getEnv("PS1") === "asdf gaming moment",
      "PS1 env var  with quotes set correctly"
    );
  });
  ...
  it("prints to stderr on error", () => {
    ...
    assert(
      shell.run("bruhMoment").streams.stderr ===
        "bruhMoment: command not found",
      "Prints command not found to stderr"
    );
    assert(
      shell.run("/a").streams.stderr === "/a is not executable",
      "Prints that a is not executable to stderr"
    );
  });
  ...
  it("replaces a wildcard", () => {
    ...
    assert(
      rootChildren.reduce((a, x) => a && rootReplacedResult.match(x)?.length),
      "Has all root children"
    );

    const bChildren = ["/a/b/c", "/a/b/d", "/a/b/e", "/a/b/echo"];
    let bReplacedResult = shell.run("echo /a/b/*").streams.stdout;
    assert(
      bChildren.reduce((a, x) => a && bReplacedResult.match(x)?.length),
      "Has all of a/b's children"
    );
    ...
    assert(
      fChildren.reduce((a, x) => a && fReplacedResult.match(x)?.length),
      "Uses current working directory to traverse children"
    );
  });
  ...
  it("builds classed prompts", () => {
    ...
    shell.setEnv(
      "PS1",
      "<blue>${USER}</blue><orange>@</orange><purple>${HOSTNAME}</purple>:${PWD} <green>$</green>"
    );
    assert(
      shell.buildPrompt() ===
        '<span class="blue">test</span><span class="orange">@</span><span class="purple">localhost</span>:/ <span class="green">$</span>',
      "Prompt is correct"
    );
  });
};
